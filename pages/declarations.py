        ...
        [
            html.Div(
                ...
                children=[
                    ...
                    html.P(
                        "Examinez chaque déclaration et validez ou rejetez-la pour qu'elle apparaisse dans les statistiques.",
                        style={"color": "#6B6B6B", "marginBottom": "28px"},
                    ),
                    html.Div(id="declarations-store", style={"display": "none"}),
                    html.Div(id="declarations-list"),
                    dcc.Interval(
                        id="declarations-refresh", interval=30000, n_intervals=0
                    ),
                ],
            ),
        ]
...
def _case_card(case):
    """Construit la carte d'un cas pending."""
    status_label, status_color = STATUS_LABELS.get(
        case.status, ("Inconnu", "secondary")
    )
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            dbc.Badge(
                                status_label, color=status_color, className="mb-2"
                            ),
                            html.H5(
...
def update_declarations(n_intervals, validate_clicks, reject_clicks):
    ...
    # Traitement d'une action valider/rejeter
    if isinstance(triggered, dict):
        case_id = triggered["index"]
        action = triggered["type"]
        new_status = "validated" if action == "btn-validate" else "rejected"
        db = get_session()
        try:
