...
from config import REGIONS, PREFECTURES, TYPES_VIOLENCE, NUMERO_VBG, NUMERO_POLICE
from services.cases_service import create_case
from database import get_session

dash.register_page(
    __name__,
    path="/espace/signaler",
    title="Signaler un cas – VBG Guinée",
    order=5,
)
            ...
            children=[
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div([
                                    html.H4("Localisation du cas", className="section-title"),
                                    html.P("Région et préfecture où l'incident s'est produit.", className="section-subtitle"),
                                ]),
                            ],
                        ),
                        dbc.Row([
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div([
                                    html.H4("Nature du cas", className="section-title"),
                                    html.P("Type de violence et date de l'incident.", className="section-subtitle"),
                                ]),
                            ],
                        ),
                        dbc.Row([
                            ...
                            dbc.Col([
                                ...
                                dbc.Input(id="f-date", type="date", max=str(date.today()), className="form-control"),
                            ], md=4),
                        ], className="g-3"),
                    ],
                ),
                html.Div(style={"height": "20px"}),
                # Section 3 : Profil victime
                html.Div(
                    className="form-card",
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div([
                                    html.H4("Profil de la victime", className="section-title"),
                                    html.P("Informations démographiques anonymisées.", className="section-subtitle"),
                                ]),
                            ],
                        ),
                        dbc.Row([
                            ...
                            dbc.Col([
                                ...
                                dbc.Input(
                                    id="f-age", type="number", min=0, max=120,
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        dbc.Button("Soumettre le cas", id="btn-submit", n_clicks=0, className="btn-primary-vbg"),
                        html.Div(
                            className="urgency-banner",
                            style={"marginTop": "24px"},
                            children=[
                                html.Strong("Si la victime est en danger immédiat :"),
...
def update_prefectures(region):
    if not region:
        return [], True
    return [{"label": p, "value": p} for p in PREFECTURES.get(region, [])], False
...
def submit_case_callback(n_clicks, region, prefecture, type_v, date_str, gender, age):
    errors = []
    if not region:      errors.append("La région est obligatoire.")
    if not prefecture:  errors.append("La préfecture est obligatoire.")
    if not type_v:      errors.append("Le type de violence est obligatoire.")
    if not date_str:    errors.append("La date de l'incident est obligatoire.")
