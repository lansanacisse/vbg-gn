...
def layout():
    ...
    return html.Div([
        ...
        html.Div(className="hero", children=[
            html.H1("VBG Guinée"),
            html.P("Plateforme nationale de recensement des violences basées sur le genre"),
            html.Blockquote(
        ...
        html.Div(className="filters-section", children=[
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{"label": "Toutes", "value": "all"}]
                              + [{"label": str(y), "value": y} for y in reversed(years)],
                        value="all",
                        clearable=False,
                    ),
                ], md=4),
                ...
                dbc.Col([
                    html.Div(id="kpi-total", style={"marginTop": "22px"}),
                ], md=4),
            ], className="g-3"),
        ]),
...
def update_dashboard(year, region):
    ...
    kpi = html.Div(className="kpi-card", children=[
    ...
    if by_region:
        fig = px.bar(x=list(by_region.keys()), y=list(by_region.values()),
                     labels={"x": "R\u00e9gion", "y": "Nombre de cas"},
                     title="Cas par r\u00e9gion", color_discrete_sequence=[ACCENT])
        fig.update_layout(paper_bgcolor=CARD_BG, plot_bgcolor=CARD_BG,
                          font={"family": "Inter, sans-serif"}, title_font_color=PRIMARY,
                          margin=dict(t=44, b=12, l=12, r=12))
        chart_region = dcc.Graph(figure=fig, config={"displayModeBar": False})
    else:
        chart_region = html.P("Aucune donn\u00e9e.", style={"color": "#999", "padding": "32px", "textAlign": "center"})

    by_type = stats["by_type"]
    if by_type:
        colors = [PRIMARY, ACCENT, "#C0392B", "#1A5276", "#117A65", "#7D6608", "#512E5F"]
        fig2 = go.Figure(go.Pie(labels=list(by_type.keys()), values=list(by_type.values()),
                                hole=0.4, marker={"colors": colors[:len(by_type)]}))
        fig2.update_layout(paper_bgcolor=CARD_BG, font={"family": "Inter, sans-serif"},
                           title="Type de violence", title_font_color=PRIMARY,
                           margin=dict(t=44, b=12, l=12, r=12),
                           legend={"orientation": "v", "font": {"size": 11}})
        chart_type = dcc.Graph(figure=fig2, config={"displayModeBar": False})
    else:
        chart_type = html.P("Aucune donn\u00e9e.", style={"color": "#999", "padding": "32px", "textAlign": "center"})
